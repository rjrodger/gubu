...
describe('expr', () => {
  ...
  test('desc-call-order', () => {
    ...
    let gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Min(1)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min'])

    g = Gubu({ a: Max(1) })
    expect(g({ a: 1 })).toEqual({ a: 1 })
    gs = g.stringify(null, true)
    expect(gs).toEqual('{"a":"Max(1)"}')
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Max(1)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Max'])
    ...
    gr = Gubu.expr(JP(gs).a)
    // console.log(gr)
    expect(gr.s).toEqual('Max(3).Min(1)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Max', 'Min'])
    ...
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Min(1).Max(3)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])

    g = Gubu({ a: Required(Max(3, Min(1))) })
    expect(g({ a: 2 })).toEqual({ a: 2 })
    gs = g.stringify(null, true)
    // console.log(gs)
    expect(gs).toEqual('{"a":"Min(1).Max(3).Required()"}')
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Min(1).Max(3).Required()')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])
    ...
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Required().Min(1).Max(3)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])
    expect(gr.r).toEqual(true)
    ...
    gs = g.stringify(null, true)
    // console.log(gs)
    expect(gs).toEqual('{"a":"Default(2).Min(1).Max(3)"}')
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Default(2).Min(1).Max(3)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])
    expect(gr.r).toEqual(false)
    expect(gr.f).toEqual(2)
    ...
    gs = g.stringify(null, true)
    // console.log(gs)
    expect(gs).toEqual('{"a":"Required().Default(2).Min(1).Max(3)"}')
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Required().Default(2).Min(1).Max(3)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])
    expect(gr.r).toEqual(false)
    expect(gr.f).toEqual(2)
    ...
    gs = g.stringify(null, true)
    // console.log(gs)
    expect(gs).toEqual('{"a":"Default(2).Required().Min(1).Max(3)"}')
    gr = Gubu.expr(JP(gs).a)
    expect(gr.s).toEqual('Default(2).Required().Min(1).Max(3)')
    expect(gr.b.map((f: any) => f.name)).toEqual(['Min', 'Max'])
    expect(gr.r).toEqual(true)
    expect(gr.f).toEqual(2)
